    ...
    $( "#entity_name" ).autocomplete({
      source: <?php echo $source;?>,
       select:function(evt, ui)
        { 
        	  $( "#entity_name" ).val( ui.item.entity_name );
...
<div class="row">
  ...
  ?>
	...
	</div><?php if(!empty($this->input->post('sub'))){ ?>
									<div class="col-xs-12 " id="t">
										<h3 class="header smaller lighter blue">Fill the information </h3>
										...
										<div>
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												<form action="<?php echo base_url(''.$user_type.'/company/attachAct')?>" method="POST">
												<thead>
													<tr>
														<th class="center">
															<label class="pos-rel">
				 	...
				 	if ($getActs->num_rows() >0) {
				 		foreach ($getActs->result() as $key ) {
				 			$getCheckAct=$this->db->distinct()->select(' act_code')->from('act_applicable_to_customer')-> where(' custid',$this->input->post('custid'))-> where(' act_code',$key->act_code)->get();	 			
				 			echo "<tr><td>";
				 			if ($getCheckAct->num_rows()>0 ) {?>				
				 					
				 				 <input type="checkbox"  name="actCode[]" id="act_code[]"  value="<?php echo $key->act_code;?>" disabled="disabled" checked/>	
				 			<?php }
				 			else
				 			{?>
				 				<input type="checkbox"  name="actCode[]" id="act_code[]"  value="<?php echo $key->act_code?>"/>	
				 			<?php }?>
				 			</td>
                              <td bgcolor="#fff"><?php echo $key->act;?></td>
                              <td bgcolor="gray"><?php echo $key->act_code;?></td>
...
<?php 
					...
					?>
												...
												<input type='hidden' name='compName' value='<?php echo $this->input->post('name');?>' >
